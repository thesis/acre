...
/**
 * The redemption itself never calls this contract directly - the user's Safe
 * calls `acreBTC.approveAndCall(bitcoinRedeemer, shares, redemptionData)` and
 * the redeemer picks it up in `receiveApproval`. This wrapper therefore only
 * needs the deployed address; the artifact is not used to encode calldata,
 * which is why an ABI change on the redeemer does not ripple into the SDK.
 */
