...
contract Allocator is Ownable {
  ...
  constructor(ERC20 _tBTC, stBTC _stBTC, IReserve _reserve) Ownable(msg.sender) {
    tBTC = _tBTC;
    stbtc = _stBTC;
    reserve = _reserve;
  }
  ...
  function addStrategy(
    ...
    require(strategies[_strategy].baseToken == address(0), 'STRATEGY_EXISTS');
    ...
    if (_strategyData.baseRatio > 0) {
      _approve(_strategyData.baseToken, address(_strategy), type(uint256).max);
      reserve.approve(
  ...
  function _strategyTotalAssets() internal view returns (uint256 totalAmount) {
  ...
  function _allocate(
    ...
    for (uint256 i = 0; i < strategyList.length; i++) {
      ...
      if (isMinimalStrategy) {
        ...
        if (_settlement) {
          strategyInfo.lastTotalAssets = uint128(strategy.totalAssets());
        }
      } else {
        /*=============================================
                        LP4626 Specific
        =============================================*/
        uint256 baseAmount = amountsList.length > 0 ? amountsList[i][1] : 0;
        ...
        if (baseAmount > 0) {
          ...
          strategyInfo.netFlow += int128(
            int256(quoteAmount + lpStrategy.convert(baseAmount, true))
          );
        } else {
          strategyInfo.netFlow += int128(int256(quoteAmount));
        }
  ...
  function _free(
    ...
    for (uint256 i = 0; i < strategyList.length; i++) {
      ...
      if (allocatorShares > 0) {
        ...
        uint256[] memory amounts = new uint256[](isMinimalStrategy ? 1 : 2);
        amounts[0] = _balanceOf(address(tBTC), address(this));

        if (isMinimalStrategy) {
          strategy.redeem(
            _ratio >= 1e4 ? allocatorShares : (allocatorShares * _ratio) / 1e4,
            address(this),
            address(this)
          );
          amounts[0] = _balanceOf(address(tBTC), address(this)) - amounts[0];
          ...
          amounts[0] = _balanceOf(address(tBTC), address(this)) - amounts[0];
          amounts[1] = _balanceOf(baseTokenAddr, address(this)) - amounts[1];
          currentTotalAsset = amounts[0] + lpStrategy.convert(amounts[1], true);
          /*==========/ LP4626 Specific =================*/
        }

        amountsList[i] = amounts;
        strategies[strategy].netFlow -= int128(int256(currentTotalAsset));
        netPerformance += (int256(currentTotalAsset) - compareTotalAsset);
      }
      ...
      strategies[strategy].pricePerShare = uint128(strategy.pricePerShare());
    ...
    for (uint256 i; i < amountsList.length; i++) {
      ...
      if (strategies[strategyList[i]].baseRatio > 0) {
        ...
        if (amountsList[i][1] > 0) {
          currTotalAssets += ILP4626(address(strategyList[i])).convert(
            amountsList[i][1],
            true
          );
        }
  ...
  function _swap(uint256[][] memory amountsList) internal {
    for (uint256 i = 0; i < amountsList.length; i++) {
      ...
      if (baseAmount > 0) {
        uint256 baseLeft = reserve.swap(lpStrategy, address(tBTC), baseAmount);
        // swap base token to quote token via uniswap v3 pool
        if (baseLeft > 0) {
          _transfer(lpStrategy.baseToken(), address(lpStrategy), baseLeft);
  ...
  // dev/testing env only fallback method, in case of emergency
  ...
  function invoke(
    ...
    require(_addr != address(0), 'ZERO_ADDR');
    // slither-disable-next-line controlled-delegatecall
    (bool success, ) = _preserveContext
      ? _addr.delegatecall(_data)
      : _addr.call(_data);
    require(success, 'UnexpectedError');
  }
}